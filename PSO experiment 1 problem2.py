...
class PSO(object):
    ...
    def calculate_fitness(self, x):
        return -20*np.exp(-0.2*pow(np.sum(np.square(x), axis=1)/self.dim,1/2))-np.exp(np.sum(np.cos(2*np.pi*x/180),axis=1)/self.dim)+20+np.e
...
plt.savefig("D:\demo\demo4\PSO problem 2")

work_book.save('PSO problem2.xls')
# plt.show()