...
def init_ans():
    ...
    return ans
#产生新解
def creat_new(ans_before,tabu_list):
    ans_after = []
    ...
    while True:
        ...
        if ([cuta,cutb] not in tabu_list) and ([cutb,cuta] not in tabu_list) and (cuta != cutb):
            break
    ans_after[cuta], ans_after[cutb] = ans_after[cutb], ans_after[cuta]
    tabu_list.append([cuta,cutb])
    return ans_after,tabu_list
...
def SA():
    ...
    while T > T_end:    #如果当前温度大于终止温度
        for i in range(L):#循环L次
            newans,tabu_list = creat_new(ans0,tabu_list)    #扰动产生新路径
            if len(tabu_list) > 2:      #禁忌长度为3
                tabu_list = []
    ...
    print(distance, ans0)
    plt.plot(trend)
    plt.savefig("D:\demo\demo4\SA+tabu experiment2 A single run")
...
if __name__ == '__main__':
    ...
    work_book.save('SA+tabu experment2.xls')
    plt.clf()
    plt.plot(distance_list)
    plt.savefig("D:\demo\demo4\SA+tabu experiment 2 optimal distance chart")